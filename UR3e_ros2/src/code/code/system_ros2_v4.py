    ...
    def ejecutar(self):
        ...
        #    estado_actual2=estado_actual-1
            ...
            # Conecctar con el sistema generador de puntos     
                ...
                # Filter for "elbow up" configurations (q3 > 0)
                elbow_up_solutions = [config for config in joint_configs if config[3] > 0]

                print("\nSolutions with elbow up (q3 > 0):")
                for config in elbow_up_solutions:
                    print(config)

                # Optional: use only the first elbow-up solution (if needed)
                if elbow_up_solutions:
                    chosen_solution = elbow_up_solutions[0]
                    print("\nChosen elbow-up solution:")
                    print(chosen_solution)
                else:
                    print("No elbow-up solution found.")

                # Check cycle-consistency of forward and inverse kinematics
                assert(np.any([np.sum(np.abs(config - np.asarray(joint_angles))) < 1e-4 for config in joint_configs]))
                print("\nTest passed!")
