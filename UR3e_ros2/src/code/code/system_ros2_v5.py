...
class SystemNode(Node):
    ...
    #________________
        ...
        for i in range(1, cont_q + 1):
            ...
            T = m_efector[i - 1]   # puedes agregar un desplazamiento a las posiciones articulares del endoscopio
          
            print(T)
            
            # Inicializar la cinemática del UR5
            ur5_kin = ikfastpy.PyKinematics()
            n_joints = ur5_kin.getDOF()


            # Extraer la parte 3x4 de la matriz (rotación y traslación)
            T_3x4 = T[:3, :]  # 3x4

            # Ejecutar cinemática inversa
            joint_configs = ur5_kin.inverse(T_3x4.reshape(-1).tolist())
            n_solutions = int(len(joint_configs) / n_joints)

            if n_solutions == 0:
                print("No se encontraron soluciones de cinemática inversa.")
            else:
                joint_configs = np.asarray(joint_configs).reshape(n_solutions, n_joints)

                # Filtrar configuraciones con "codo arriba" (q3 > 0)
                elbow_up_solutions = [config for config in joint_configs if config[3] > 0]

                if elbow_up_solutions:
                    chosen_solution = elbow_up_solutions[0]
                    print("\nSolución seleccionada (codo arriba):")
                else:
                    chosen_solution = joint_configs[0]
                    print("\nNo se encontraron soluciones con codo arriba. Usando la primera disponible:")
                ...
                self.home_position = chosen_solution
