...
import sys
# Ruta al directorio actual donde está este script
...
class SystemNode(Node):
    ...
    def send_joint_state_robot(self, joint_positions, publisher, msg):
    ...
    def hacer_puntada_virtual(self):
    ...
    #________________
        ...
        if not hasattr(self, 'soluciones_virtuales') or not self.soluciones_virtuales:
            print("No hay soluciones guardadas para reproducir.")
            return

        for i, q_sol in enumerate(self.soluciones_virtuales):
            print(f"Publicando punto virtual {i+1}:", q_sol)
            self.send_joint_state_robot(q_sol, self.joint_state_pub, self.joint_state_msg)
    ...
    def hacer_puntada_alin(self, m_efector, m_q_vec_ik, despl):
        ...
        cont_q = len(m_efector)
        print("Número de puntos:", cont_q)   
       
        self.home_position = [0, -np.pi/2, np.pi/2, -np.pi/2, -np.pi/2, 0]  # Posicion inicial
        self.send_joint_state_robot(self.home_position, self.joint_state_pub, self.joint_state_msg)
        
        # Inicializar la cinemática
        ...
        #Arreglo soluciones
        self.soluciones_virtuales = []
        
        for i in range(cont_q):
            posES = Pose()
            T = m_efector[i]  # Matriz de transformación homogénea
            
            # Cinemática inversa
            T_3x4 = T[:3, :].reshape(-1).tolist()
            joint_configs = ur5_kin.inverse(T_3x4)
            n_solutions = len(joint_configs) // n_joints
            
            if n_solutions == 0:
                print(f"Advertencia: No se encontraron soluciones para el punto {i+1}")
                continue
                
            joint_configs = np.array(joint_configs).reshape(n_solutions, n_joints)
            
            # Filtrar y seleccionar la mejor solución
            best_solution = None
            ...
            for config in joint_configs:                    
                ...
                distance = np.linalg.norm(config - current_position)
                if distance < min_distance:
                    min_distance = distance
                    best_solution = config
                    
            # Si no hay solución con codo arriba, usar la más cercana válida
            ...
            if best_solution is None:
                print(f"Error: No hay soluciones válidas para el punto {i+1}")
                continue
                
            print(f"Solución seleccionada para punto {i+1}:", best_solution)
            
            self.soluciones_virtuales.append(best_solution.tolist())
            #self.send_joint_state_robot(best_solution, self.joint_state_pub, self.joint_state_msg)
            current_position = best_solution  # Actualizar posición actual
    ...
    def publicar_endo(self,m_q_vec_ik,m_efector2,modo):
     ...
     #  rospy.init_node('publicador', anonymous=True) #inicia el nodo publicador
       ...
       # print("Punto de interrupción")
        ...
        if modo==1:
            ...
            a1 = fila[0]
            a1 = self.th_to_val_join_roll(a1)

            a2 = fila[1]
            a2 = self.th_to_val_join_th2(a2)
            a3 = fila[2]
            a3 = self.th_to_val_join_th3(a3)
            
            pactual=self.get_status()
            pm1=pactual[0]      
            pm2=pactual[1]      
            pm3=pactual[2]              
            ...
            for i in range(1, frac+1):  # Nota que 'range' es inclusivo en el inicio y exclusivo en el fin, así que debes usar 21 para incluir el 20.
                ...
                th3pp=self.compensar_th3(pdm2,pdm3)    
        ...
        #            print(fila)
            #        fila = m_q_vec_ik[i]
               #     print("ciclo",i, " conte",fila)
                    a1 = fila[0]
                    a1 = self.th_to_val_join_roll(a1) #+100  # ajuste para penetracion **
                    a2 = fila[1]
                    a2 = self.th_to_val_join_th2(a2)
                    a3 = fila[2]
                    a3 = self.th_to_val_join_th3(a3)
     ...
     #           yaw.publish(a2)
                ...
                a2=1430
                th3pp=self.compensar_th3(a2,a3)    
         #       print(th3pp)
            ...
            #        pdb.set_trace()       # funcion para interrumpir y debugar  ==============================================
             ...
             #       habilitar un contador para repetir el mismo estado pero otra puntada
              ...
              #  poseArrayMsg2= PoseArray()
                ...
                print("EJECUTANDO PUNTADA:::::::::::::::::::::::::::::::::::::::::::::::::")
                desplazamiento=0
                #self.node.hacer_puntada_virtual(m_efector,m_q_vec_ik,desplazamiento)
                # Mover a posición de calibración (home)
                self.node.hacer_puntada_alin(m_efector,m_q_vec_ik,desplazamiento)
                self.node.hacer_puntada_virtual()
                



#                self.node.hacer_puntada(m_efector,m_q_vec_ik,desplazamiento)
